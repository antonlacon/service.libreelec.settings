...
class system(modules.Module):
    ...
    def set_keyboard_layout(self, listItem=None):
        ...
        if self.keyboard_layouts == True:
            ...
            if path_to_bmap:
                command = 'loadkmap'
                log.log(f'Executing {command} < {path_to_bmap}', log.INFO)
                os_tools.execute(command, read_input=path_to_bmap)
            else:
    ...
    def set_hw_clock(self):
        ...
        if os.path.exists('/proc/driver/rtc'):
            os_tools.execute(f'{self.SET_CLOCK_CMD}', include_stderr=False)
