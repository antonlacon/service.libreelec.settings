...
class updates(modules.Module):
    ...
    def get_hardware_flags_x86_64(self):
        ...
        gpu_path = os_tools.execute(f'/usr/bin/udevadm info --name=/dev/dri/{gpu_card} --query path', get_result=True, include_stderr=False).replace('\n','')
        log.log(f'gpu path: {gpu_path}', log.DEBUG)
        if gpu_path:
            drv_path = os.path.dirname(os.path.dirname(gpu_path))
            props = os_tools.execute(f'/usr/bin/udevadm info --path={drv_path} --query=property', get_result=True, include_stderr=False)
            if props:
